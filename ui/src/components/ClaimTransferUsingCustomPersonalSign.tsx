...
export default function ClaimTransfer(props: ITransferDetails) {
  ...
  const onClick = async () => {
    ...
    if (metaTxEnabled) {
      ...
      const contractInterface = new ethers.utils.Interface(RelayerWalletABI);
      
      // let functionSignature = contractInterface.encodeFunctionData("withdrawEther", [
      //   props.id,
      // ]);
      let functionSignature;
      if(props.mode === 0) {
        functionSignature = contractInterface.encodeFunctionData("withdrawEther", [
          props.id,
        ]);
      }else {
        functionSignature = contractInterface.encodeFunctionData("withdrawToken", [
          props.id,
        ]);
      }
